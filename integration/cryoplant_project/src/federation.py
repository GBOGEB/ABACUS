"""
Federation assimilation stub for GBOGEB/cryoplant-project (DELTA_1 auxiliary plane).

This module mirrors what GBOGEB/cryoplant-project should expose at
``src/federation.py``. It is kept in ABACUS under
``integration/cryoplant_project/src/federation.py`` as the authoritative
integration record and can be copied verbatim into the cryoplant-project
repository, same convention as integration/codespace_jyperter/src/federation.py.

Provides the assimilate() entry point consumed by:
  - scripts/recursive_build.py --smoke --index GLOBAL_index.json (ABACUS side)
...
The global federation manifest lives at:
  ...
  integration/cryoplant_project/federation/manifest.yaml (this integration stub)
"""
...
# When copied to cryoplant-project it will resolve to federation/manifest.yaml
...
def assimilate(context: Dict[str, Any] | None = None) -> Dict[str, Any]:
    """Entry point for cryoplant-project federation assimilation checks.
    ...
    Returns:
        dict with keys:
            ...
            repo (str): "GBOGEB/cryoplant-project"
            plane (str): "auxiliary"
