    ...
    class DOWRefactorMaster:  # pragma: no cover - fallback is patched in tests
        def __init__(self, *args: Any, **kwargs: Any):
            self.state = type(
                ...
                (),
                {
                    "current_phase": type("Phase", (), {"value": "UNKNOWN"})(),
                    "operations": [],
                    "iterations": 0,
                },
            )()
...
class RefactoringEngine(Protocol):
    ...
    def initialize(
        self, workspace_root: Path, config: dict[str, Any] | None = None
    ) -> bool:
        pass
...
class BaseRefactoringEngine:
    ...
    def execute(self) -> RefactoringResult:
        return RefactoringResult(
            self.name, "success" if self.validate() else "error", 0.0
        )
...
class DOWRefactorEngineAdapter(BaseRefactoringEngine):
    ...
    def initialize(
        ...
        self._dow_master = DOWRefactorMaster(
            workspace_root=Path(workspace_root), **dict(config or {})
        )
        return True
    ...
    def execute(self) -> RefactoringResult:
        ...
        try:
            ...
            return RefactoringResult(
                self.name, "success", perf_counter() - start, metrics=metrics
            )
        except Exception as exc:
            return RefactoringResult(
                self.name, "error", perf_counter() - start, errors=[str(exc)]
            )
...
class CanonicalRefactoringEngineAdapter(BaseRefactoringEngine):
    ...
    def initialize(
        ...
        super().initialize(workspace_root, config)
        self._canonical_refactorer = CanonicalDocumentRefactorer(
            workspace_root=Path(workspace_root), **dict(config or {})
        )
        return True
    ...
    def execute(self) -> RefactoringResult:
        ...
        if (
            not self.validate()
            or self._canonical_refactorer is None
        ...
        try:
            files = PathUtilities.scan_files(
                self.workspace_root, ["*.md"], recursive=True
            )
            for path in files:
...
class RefactoringEngineFactory:
    ...
    def create_engines(
        ...
        config: dict[str, Any] | None = None,
        logger: Any = None,
    ) -> list[RefactoringEngine]:
        return [
            RefactoringEngineFactory.create_engine(
                item, workspace_root, config=config, logger=logger
            )
