...
# v24 addition: PCA / Structure tab data + the 4th weight scenario, both
# generated by their own named scripts (compute_pca.py, compute_
# weight_scenario4.py) rather than recomputed here -- this script's job is
# to export the workbook's own SSOT plus these two pre-computed, discoverable
# analyses, never to duplicate the computation itself. Looked up next to IN;
# silently omitted from the export (not a crash) if not found, since not
# every workbook version this script runs against will have them yet.
import glob
_in_dir = os.path.dirname(os.path.abspath(IN))
_pca_candidates = sorted(glob.glob(os.path.join(_in_dir, "pca_results_v*.json")))
_scen4_candidates = sorted(glob.glob(os.path.join(_in_dir, "weight_scenario4_v*.json")))
PCA_JSON = _pca_candidates[-1] if _pca_candidates else None
SCEN4_JSON = _scen4_candidates[-1] if _scen4_candidates else None
...
for r in range(6, ws.max_row + 1):
    ...
    rtmRanking.append({
        ...
        # btLambda (col 24) existed in the workbook but was never exported --
        # a real gap flagged in NEXT_ITERATION_BACKLOG.md item 10 ("BT lambda
        # index -- a real third metric, currently NOT exported to the
        # Navigator"). Closing it here with no other change to this row.
        "weightedS": cellv(ws, r, 22), "btWin": cellv(ws, r, 23), "btLambda": cellv(ws, r, 24),
        "primaryDim": cellv(ws, r, 25),
...
for r in range(6, ws.max_row + 1):
    ...
    offerRanking.append({
        ...
        "weightedS": cellv(ws, r, 15), "btWin": cellv(ws, r, 16), "btLambda": cellv(ws, r, 17),
        "primaryDim": cellv(ws, r, 18),
...
# v24 addition: PCA / Structure tab source data (Proposal A scatter + domain
# quadrant table + loadings/variance, all from compute_pca.py's own output --
# never recomputed inline here) and the 4th weight scenario (compute_
# weight_scenario4.py). Both omitted (empty dict) rather than erroring if the
# JSON isn't present next to IN, so this script still runs against older
# workbook versions that predate them.
pca = {}
if PCA_JSON:
...
DATA = {
    ...
    "pca": pca, "weightScenario4": weightScenario4,
}
...
print(f"wrote {OUT}: {len(rtmRanking)} rtmRanking, {len(offerRanking)} offerRanking, "
      ...
      f"pca={'yes' if pca else 'no'}, weightScenario4={'yes' if weightScenario4 else 'no'}, "
