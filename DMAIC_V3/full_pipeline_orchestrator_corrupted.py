...
class FullPipelineOrchestrator:
    ...
    def execute_full_pipeline(self, iteration: int = 1) -> bool:
        ...
        try:
            # Start detector if available (non-blocking)
            if hasattr(self, "bg_change_detector") and self.bg_change_detector is not None:
                try:
                    self.bg_change_detector.start()
                except Exception as e:
                    print(f"[WARN] Failed to start BackgroundChangeDetector: {e}")
                    # Continue without background detection
=======
            # Phase 0: Initialization
            ...
            try:
                ...
                # If the detector was not started or is unavailable, ignore stop errors
                pass
