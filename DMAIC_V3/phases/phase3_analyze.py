...
class Phase3Analyze:
    ...
    def identify_high_complexity_files(
        self, metrics: Dict, top_n: int = 20
    ) -> List[Dict]:
            ...
            complexity_files.append(
                {
                    ...
                    "functions": (
                        functions_val
                        if isinstance(functions_val, int)
                        else len(functions_val)
                    ),
                    "classes": (
                        classes_val
                        if isinstance(classes_val, int)
                        else len(classes_val)
                    ),
                }
            )
    ...
    def run(self, iteration: int) -> Tuple[bool, Dict[str, Any]]:
        ...
        analysis_result = {
            ...
            "summary": {
                ...
                "critical_issues": len(
                    [rc for rc in root_causes if rc["severity"] == "critical"]
                ),
                "high_issues": len(
                    [rc for rc in root_causes if rc["severity"] == "high"]
                ),
                "medium_issues": len(
                    [rc for rc in root_causes if rc["severity"] == "medium"]
                ),
            },
            "analysis": {
