                ...
                if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                    if ast.get_docstring(node) is None:
                        ...
                        if isinstance(node, ast.FunctionDef):
                            docstring = (
                                f'{indent}    """TODO: Add function description"""\n'
                            )
                        else:
                            docstring = (
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    for alias in node.names:
                        imports.append(
                            (node.lineno, alias.name if alias.name != "*" else module)
                        )
...
class Phase4Improve:
    ...
    def implement_improvements(
        self,
        high_complexity_files: List[Dict],
        ...
        for improvement_type, file_path in files_to_improve[:max_files]:
            ...
            if improvement_type == "complexity":
                ...
                if r_unused.get("success"):
                    ...
                    implemented["total_modifications"] += r_unused.get(
                        "modifications", 0
                    )
                    print(
                        f"    [+] Removed {r_unused.get('modifications', 0)} unused imports"
                    )
        ...
        print(
            f"   - Unused imports removed: {len(implemented['unused_imports_removed'])}"
        )
    ...
    def generate_improvement_metrics(self, roadmap: Dict) -> Dict:
        ...
        metrics = {
            ...
            "estimated_total_effort": sum(
                (
                    3
                    if task.get("estimated_effort") == "High"
                    else 2 if task.get("estimated_effort") == "Medium" else 1
                )
                for tasks in roadmap.values()
                for task in tasks
            ),
        }
    ...
    def apply_improvements(
        self, tasks: List[Dict], per_task_file_limit: int = 5
    ) -> List[Dict]:
        """
        ...
        py_files = (
            list(self.workspace_root.rglob("*.py"))
            ...
            results.append(
                {
                    ...
                    "applied_to_files": [
                        r.get("file") for r in applied if r.get("file")
                    ],
                    "details": applied,
                }
            )
    ...
    def run(self, iteration: int) -> Tuple[bool, Dict[str, Any]]:
        ...
        implementation_results = self.implement_improvements(
            high_complexity_files[:50], low_doc_files[:50], max_files=100
        )
        ...
        improvement_result = {
            ...
            "summary": {
                ...
                "files_actually_improved": implementation_results[
                    "total_files_improved"
                ],
                "total_modifications_made": implementation_results[
                    "total_modifications"
                ],
            },
...
if __name__ == "__main__":
    ...
    iteration = (
        int(sys.argv[sys.argv.index("--iteration") + 1])
    ...
    if "phase" not in result or result.get("error"):
        print(
            f"[ERROR] Phase 4 execution failed: {result.get('error', 'Unknown error')}"
        )
        sys.exit(1)
