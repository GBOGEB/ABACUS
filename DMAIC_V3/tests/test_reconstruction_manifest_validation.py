...
def _load_reconstruction_manifest():
    manifest_path = ROOT_DIR / "docs/api/phase2_reconstruction_manifest.json"
    if not manifest_path.exists():
        pytest.skip(
            "docs/api/phase2_reconstruction_manifest.json is not present in this checkout; "
            "commit the manifest under docs/api/ or generate it before running validation tests."
        )
    return json.loads(manifest_path.read_text(encoding="utf-8"))


def test_reconstruction_manifest_in_repo_is_valid():
    payload = _load_reconstruction_manifest()
