...
class TestPhase5Control:
    ...
    def test_validation_checkpoints(self, phase5, phase4_output):
        ...
        assert (
            "validation_checkpoints" in result
            or "checkpoints" in result
            or "quality_gates" in result
        )
    ...
    def test_output_structure(self, phase5, phase4_output):
        ...
        assert "timestamp" in result
        assert (
            "input_source" in result or "phase" in result
        )  # input_source may not always be present
    ...
    def test_multiple_iterations(self, phase5, config):
        for iteration in [1, 2]:
            output_dir = (
                config.paths.output_root / f"iteration_{iteration}" / "phase4_improve"
            )
            output_dir.mkdir(parents=True, exist_ok=True)
