...
class TemporalMetadataEngine:
    ...
    def __init__(self, workspace_root: Path, db_path: Optional[Path] = None):
        ...
        self.db_path = db_path or (
            self.workspace_root / ".dmaic" / "temporal_metadata.db"
        )
    ...
    def _analyze_python_file(
        ...
        try:
            ...
            dependencies = [
                imp.split()[1].split(".")[0] for imp in imports if "import" in imp
            ]
    ...
    def create_digital_twin(self) -> DigitalTwinState:
        ...
        quality_metrics = {
            ...
            "python_file_ratio": (
                sum(1 for f in files if f.file_type == FileType.PYTHON) / len(files)
                if files
                else 0
            ),
            "test_coverage": (
                sum(f.test_coverage for f in files) / len(files) if files else 0
            ),
            "avg_quality_score": (
                sum(f.quality_score for f in files) / len(files) if files else 0
            ),
        }
