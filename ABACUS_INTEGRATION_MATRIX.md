...
| --------- | ------ | ---------- | -------- | ---- |
| ExecutionBackbone | Yes: `core/execution_backbone/backbone.py` defines `KEB` and `schedule_task`. | Yes: `local_mcp/knowledge_integration_v2.3.py` imports `core.execution_backbone.ExecutionBackbone`; `DMAIC_V3/core/twelve_cluster_orchestrator.py` imports `KEB`; README maps C9-C10 runtime operations to `local_mcp/knowledge_integration_v2.3.py`. | Partial: `KnowledgeIntegrationV23` constructs `ExecutionBackbone(max_workers=2, max_memory_mb=2048)` and exposes `schedule_agent_task`; `TwelveClusterOrchestrator` constructs KEB when `EXECUTION_BACKBONE_AVAILABLE`; `.github/workflows/v23-cicd.yml` executes `python local_mcp/knowledge_integration_v2.3.py`. | Implementation SSOT: `core/execution_backbone/backbone.py`; consumption adapter: `local_mcp/knowledge_integration_v2.3.py`. |
...
### ExecutionBackbone consumption

- `local_mcp/knowledge_integration_v2.3.py` imports `KEB` from `core.keb.keb` and `GBOGEB` from `core.gbogeb.gbogeb`.
- `KnowledgeIntegrationV23.__init__` constructs `ExecutionBackbone(max_workers=2, max_memory_mb=2048)` and `GBOGEB(workspace=...)` when core modules are available.
- `KnowledgeIntegrationV23.schedule_agent_task` calls `self.keb.schedule_task(...)` through timeout protection.
- `DMAIC_V3/core/twelve_cluster_orchestrator.py` imports `KEB`, sets `EXECUTION_BACKBONE_AVAILABLE`, and constructs `ExecutionBackbone(max_workers=min(max_workers, 4), max_memory_mb=2048)` when enabled.
...
\n\n## Terminology control\n\nKEB is reserved for **Knowledge Exchange Bridge**. The local bounded task\nexecution runtime is **ExecutionBackbone** and is implemented under\n`core/execution_backbone/`.\n