...
class MarkdownReaderEngine:
    ...
    def parse_markdown(self, content: str) -> list[MarkdownElement]:
        ...
        for line in content.splitlines():
            ...
            for match in self.LINK_RE.finditer(line):
                elements.append(
                    MarkdownElement(
                        ElementType.LINK, match.group(1), {"target": match.group(2)}
                    )
                )
    ...
    def validate_link(
        self, link: str, base_path: Path | str | None
        ...
        candidate = (
            (base / link).resolve() if not Path(link).is_absolute() else Path(link)
        )
        return LinkValidationResult(
            link=link,
            valid=candidate.exists(),
            link_type="internal",
            reason="" if candidate.exists() else "missing",
        )
            ...
            for pattern in patterns:
                for path in root_path.rglob(pattern):
                    if path.is_file():
                        artifacts.append(
                            ArtifactMetadata(
                                str(path), artifact_type, path.stat().st_size
                            )
                        )
    ...
    def generate_statistics(self, root: Path | str) -> dict[str, Any]:
        ...
        counts: dict[str, int] = {}
        for path in files:
            for element in self.parse_markdown(
