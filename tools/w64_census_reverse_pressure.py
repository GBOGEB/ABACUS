...
GENERATED_TREE_COMPONENTS = {"generated", "dist", "build", "exports"}
RELEASE_CRITICAL_CATEGORIES = {
    "workflow",
    "ssot",
    "schema_or_config",
    "source_or_script",
    "binary_or_rendered_output",
}
...
def path_components(asset: dict[str, object]) -> list[str]:
    return [part.lower() for part in Path(path_text(asset)).parts]


def is_authority_like(asset: dict[str, object]) -> bool:
    text = path_text(asset).lower()
    category = str(asset.get("category", ""))
    return category in {"ssot", "schema_or_config"} or any(
        keyword in text for keyword in AUTHORITY_KEYWORDS
    )


def is_generated_like(asset: dict[str, object]) -> bool:
    if str(asset.get("classification")) == "generated":
        return True
    return any(part in GENERATED_TREE_COMPONENTS for part in path_components(asset))


def is_stale_tree_like(asset: dict[str, object]) -> bool:
    if str(asset.get("classification")) == "dormant":
        return True
    parts = path_components(asset)
    return any(
        part in STALE_TREE_COMPONENTS or part.startswith(STALE_TREE_PREFIXES)
        for part in parts
    )
...
def duplicate_authority_findings(
            ...
            findings.append(
                {
                    ...
                    "reason": (
                        "Multiple authority-like tracked assets share a duplicate "
                        "family; consolidation decision required before release credit."
                    ),
                }
            )
    return findings
...
def generated_inflation_findings(
    assets: list[dict[str, object]],
) -> list[dict[str, object]]:
    generated = [asset for asset in assets if is_generated_like(asset)]
    authority_like_generated = [
        asset for asset in generated if is_authority_like(asset)
    ]
    findings: list[dict[str, object]] = []
    if generated:
        findings.append(
            {
                ...
                "reason": (
                    "Generated outputs are inventory only unless separately lineaged "
                    "to source authority and generator command."
                ),
            }
        )
        ...
        findings.append(
            {
                ...
                "paths": sorted(
                    path_text(asset) for asset in authority_like_generated
                )[:250],
                "reason": (
                    "Generated authority-like files cannot be counted as SSOT without "
    ...
    return [
        {
            ...
            "reason": (
                "Release-critical unknown assets must be classified before W64 DoV "
                "or release readiness credit."
            ),
        }
    ]
...
def reverse_pressure(census: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "schema_version": "W64-CENSUS-P2-1.1.0",
        "purpose": (
            "Reverse-pressure census findings; no migration, deletion, or release credit."
        ),
