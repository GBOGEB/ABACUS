...
def sha256(paths: list[Path]) -> str:
 h = hashlib.sha256()
 for path in paths:
  h.update(path.as_posix().encode()); h.update(b"\0"); h.update(path.read_bytes())
 return h.hexdigest()
...
def main() -> int:
 ...
 parser.add_argument("--output", default="federation/qps/runtime/QPS_FED_W04_DOW_RUNTIME_RECEIPT.yaml")
 args = parser.parse_args()
 if not re.fullmatch(r"[0-9a-f]{40}", args.child_sha): raise SystemExit("--child-sha must be a full commit SHA")
 request, qa = load(REQUEST), load(QA)
 ...
 names = request.get("input_contract", {}).get("child_artifacts", [])
 evidence = [locate(Path(args.child_root), str(name)) for name in names]
 ...
 for operation in requested:
  ...
  status[operation] = {"stage_id": operation, "mechanic_path": "tools/run_qps_w04_dow_return.py::controlled_evidence_scan", "executed": True, "status": "PASS", "reason": f"parsed {len(evidence)} child artifacts; matched {len(hits)} governed cues"}
 ...
 out = Path(args.output); out.parent.mkdir(parents=True, exist_ok=True); out.write_text(yaml.safe_dump(receipt, sort_keys=False), encoding="utf-8")
 print(out); return 0
